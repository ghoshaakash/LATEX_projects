...
\begin{figure}[H]
	...
	\end{tikzpicture}
	\end{center}
	\caption{The Hawaiian Earring($X$)}
\end{figure}
Let $x\in X$. It is easy to see that for $x\ne0$, there exist an open set $U_x$ such that $x\in U_x$ and $U_x$ is homeomorphic to a line segment. We note that this is not true for $x=0$. This is because any open set containing $0$ contains infinitely many  $C_n$. Therefore, there is no simply connected neighborhood of $0$ and there exists no $U_0$ of the form mentioned above. 
\begin{figure}
	...
	\end{tikzpicture}
	\end{center}
	\caption{a. A neighborhood of $x\in X$, $x\ne0$ which is homeomorphic to a line segment in $\mathbb R$. b. An arbitrary neighbourhood of $x=0$}
\end{figure}
...
\begin{figure}[H]
	\begin{center}
		...
		\end{tikzpicture}
		\end{center}
		\caption{Visual represntation of $Y$.}
\end{figure}
\subsection{$Y$ as a cover of $X$}
\begin{figure*}[h]
	\begin{center}
		\tikzset{every picture/.style={line width=0.75pt}} %set default line width to 0.75pt        
		...
		\end{center}
		\caption{Visual representation of $\phi$. A point and it's image has the same colour.}
\end{figure*}
We define \textcolor{black}{$\phi:Y\to X$} as below:
\textcolor{black}{$$\phi(\alpha)=\begin{cases}
    i+e^{2\pi i\alpha-i\pi/2}&\text{ if }\alpha\in\mathbb R\\
    h&\text{if }\alpha=z+h\text{ and }\\
	&h\in \tilde H,z\in\mathbb{Z}
\end{cases}$$}
We note that $\phi$ is a covering map. Points in the outer shell have a neighbourhood which is evenly covered by $\mathbb{R}$. All the other points have a neighborhood which is evenly covered by their copies in $\tilde H$ at each $z\in\mathbb{Z}$. 
...
\begin{center}
	...
	\end{center}
	\caption{Visual representation of $Z$}
\end{figure}
...
\begin{figure}[H]
	\begin{center}
		...
		\end{center}
		\caption{$Z_n\cap f^{-1}(U)$. Note it is path connected and has two points ($z,z+2i$) where there is no neighbourhood which is homeomorphic to a line segment in $\mathbb R$. b. How $U$ might have looked like.}
\end{figure}
